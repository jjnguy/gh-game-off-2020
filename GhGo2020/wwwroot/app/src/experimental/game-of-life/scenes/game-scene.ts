...
export class GameScene extends Phaser.Scene {
  ...
  // Timer
  private timerForDrawNextGeneration: Phaser.Time.TimerEvent;
  private calculateDelay = 1;
  private timerForCalculateNextGeneration: Phaser.Time.TimerEvent;
  ...
  init(): void {
    ...
    for (let cells of this.grid.getCells()) {
      this.createField(cells.getPosition(), cells.isAlive());
    }
    ...
    this.generationQueue.push(this.grid);

    // Init timer
    this.timerForCalculateNextGeneration = this.time.addEvent({
      delay: this.calculateDelay,
      callback: this.calculateGeneration,
      callbackScope: this,
      loop: true
    });

    this.timerForDrawNextGeneration = this.time.addEvent({
      delay: 100,
  ...
  private nextGeneration(): void {
    ...
    this.generation++;
    this.generationText.setText(`Generation: ${this.generation}`);

    this.grid = this.generationQueue.shift();
    if (!this.grid) {
  ...
  private calculateGeneration(): void {
    if (this.generationQueue.length > 500) return;
    console.log('calculating generation');
    this.generationQueue.push(this.generationQueue[this.generationQueue.length - 1].evaluateRules());
  }
   ...
   */
  private createField(position: Phaser.Math.Vector2, isAlive: boolean): void {
